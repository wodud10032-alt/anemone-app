...
import { fetchConsulting, updateConsulting } from "../api";
...
export default function ConsultingDetailPage() {
  ...
  useEffect(() => {
    (async () => {
      const { data } = await fetchConsulting(id);
      setForm({
        ...data,
        width: data.width ?? "",
        height: data.height ?? "",
  ...
  const onSubmit = async (e) => {
    ...
    await updateConsulting(id, payload);
    alert("저장되었습니다.");
    navigate("/consultings");
  };

  if (!form) return <Layout>불러오는 중...</Layout>;
